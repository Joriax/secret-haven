...
serve(async (req) => {
  ...
  try {
    ...
    // ========== SET DECOY PIN ==========
      ...
      // Check if decoy PIN is already used by another user (security: generic error message)
      const { data: allUsers } = await supabase
        .from('vault_users')
        .select('id, pin_hash, decoy_pin_hash')
        .neq('id', authenticatedUser.userId);
      ...
      if (allUsers) {
        for (const otherUser of allUsers) {
          ...
          const matchesMain = await verifyPin(newPin, otherUser.pin_hash);
          if (matchesMain) {
            await logSecurityEvent(supabase, authenticatedUser.userId, 'decoy_pin_failed', { reason: 'pin_collision' }, req);
            return new Response(
              JSON.stringify({ success: false, error: 'Tarn-PIN nicht möglich. Bitte wähle einen anderen PIN.' }),
              { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 400 }
            );
          }
          // Check against decoy PIN
